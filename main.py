...
def play(speed):
    ...
    # # Play the video in reverse mode
     ...
     if not grabbed:
         exit()
     frame = imutils.resize(frame, width=SET_WIDHT, height=SET_HEIGHT)
...
def pending(decision):
    ...
    frame = cv2.cvtColor(cv2.imread("descion.jpg"),cv2.COLOR_BGR2RGB)
    frame = imutils.resize(frame, width=SET_WIDHT, height=SET_HEIGHT)
    frame = PIL.ImageTk.PhotoImage(image=PIL.Image.fromarray(frame))
    canvas.image = frame
    canvas.create_image(0, 0, image=frame, anchor=tkinter.NW)
    ...
    frame = cv2.cvtColor(cv2.imread("gada.jpg"), cv2.COLOR_BGR2RGB)
    frame = imutils.resize(frame, width=SET_WIDHT, height=SET_HEIGHT)
    frame = PIL.ImageTk.PhotoImage(image=PIL.Image.fromarray(frame))
    canvas.image = frame
    canvas.create_image(0, 0, image=frame, anchor=tkinter.NW)
...
#widht and height of your main screen
SET_WIDHT=700
SET_HEIGHT=400
...
cv_img = cv2.cvtColor(cv2.imread("thirdumpire.jpg"),cv2.COLOR_BGR2RGB)
canvas = tkinter.Canvas(window,width=SET_WIDHT, height=SET_HEIGHT)
